...
static const uint16_t IdealModClockFrqMHz = 12;
 ...
 */
static inline void SetClockDivides(float sysClkMHz, float I2CClkKHz);
...
static inline void SetClockDivides(float sysClkMHz, float I2CClkKHz)
{
	 ...
	 */
	uint16_t IPSC = (uint16_t)(sysClkMHz/IdealModClockFrqMHz);
	...
	uint16_t ICCLH = (uint16_t)(1000 * sysClkMHz/(2 * I2CClkKHz * (IPSC + 1)) - d);
