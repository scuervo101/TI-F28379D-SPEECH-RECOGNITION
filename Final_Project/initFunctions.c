...
void Init_MIC_AIC23(void) {
    ...
    SpiTransmit(command);
    SmallDelay();
    command = fullpowerup();      // Turn everything
    SpiTransmit(command);
}
...
void InitAIC23_kHz(uint16_t freq) {
    ...
    SpiTransmit(command);
    SmallDelay();
    command = CLKsampleratecontrol (freq);
    SpiTransmit(command);
    SmallDelay();
...
void InitMcBSPb(void *ISR)
{
    ...
    //Enable Interrupt
    PieVectTable.MCBSPB_RX_INT = ISR;
    PieCtrlRegs.PIEIER6.bit.INTx7 = 1;
    IER |= M_INT6;                             // Enable M_INT6 in CPU
